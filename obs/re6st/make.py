...
def bootstrap(task):
    try:
        ...
        with cwd(BUILD + "/bootstrap-dir"):
            shutil.copy2("../buildout.cfg", ".")
        ...
        with cwd(BUILD):
            check_call(('bootstrap-dir/bin/buildout', 'bootstrap'))
...
@task((upstream, buildout, original_directory, __file__,
       "Makefile.in", "cleanup", "install-eggs", "build_python3_if_needed.bash", "original_directory"))
def tarball(task):
...
def dch(task):
    with cwd("re6stnet") as p:
        ...
        check_output(("make", "-f", "-", p,
            ...
            input=open("debian/common.mk").read().replace(task.output, p).encode())
