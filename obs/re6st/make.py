...
BUILD_KEEP = "babeld", "buildout.cfg", "download-cache", "extends-cache", "bootstrap-dir", "python3.tar.xz"
...
def bootstrap(task):
    try:
        ...
        check_call(('buildout', 'buildout:download-cache=../download-cache',
            'buildout:extends-cache=../extends-cache', 'buildout:parts=networkcached',
            'buildout:relative-paths=true', 'bootstrap'),
...
@task((upstream, buildout, __file__,
       "Makefile.in", "cleanup", "install-eggs", "build_python3_if_needed.sh"))
def tarball(task):
