...
copy_and_solve_templates () {
	if [ ! -d "$1" ]; then
		echo "copy_and_solve_templates(): The source directory does not exist, returning."
		echo "$1"
		return
	elif [ ! -d "$2" ]; then
		echo "copy_and_solve_templates(): The target directory does not exist, returning."
		echo "$2"
		return
	fi
